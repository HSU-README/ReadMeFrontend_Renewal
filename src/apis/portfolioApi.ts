...
export const getPortfolio = async (docId: string) => {
  ...
  const response = await serverApi.get(`https://cors-anywhere.herokuapp.com/https://hsureadme.herokuapp.com/api/v1/doc/${docId}`);
  try {
...
export const getAllPortfolio = async () => {
  const response = await serverApi.get('https://cors-anywhere.herokuapp.com/https://hsureadme.herokuapp.com/api/v1/home/docs/all');
  try {
    return response.data.result;
  } catch (error) {
